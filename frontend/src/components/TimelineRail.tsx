...
import { clampYear, formatYearLabel, formatYearRange } from "@/utils/yearFormatting";

// Layout constants
const YEAR_WIDTH = 80; // px per year column
...
function YearColumn({
  ...
  const columnWidth = isFocus ? "w-28" : isNeighbor ? "w-20" : "w-16";
  const columnScale = isFocus
    ? "scale-105"
    : isNeighbor
      ? "scale-100"
      : "scale-95 opacity-70";
  const cardStackHeight = isFocus
  ...
  return (
    <div
      className={`flex flex-col items-center shrink-0 transition-all duration-300 ${columnWidth} ${columnScale}`}
    >
      {/* Cards area */}
      <div
        className={`w-full space-y-2 pb-2 scrollbar-hide ${cardStackHeight}`}
      >
...
export function TimelineRail({
  ...
  return (
    <div aria-label="Timeline rail" className="space-y-4">
      {/* Header */}
      <header className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
        <div>
          ...
          <h2 className="text-sm font-semibold text-slate-50">
            Events around {formatYearLabel(focusYear)}
          </h2>
      ...
      <div
        ref={railRef}
        className="relative flex cursor-grab gap-1 overflow-x-auto px-4 py-4 scrollbar-hide active:cursor-grabbing"
        role="slider"
        tabIndex={0}
